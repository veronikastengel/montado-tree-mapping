...
Usage:
    python scripts/02b_treetop_detection_lidar.py
    python scripts/02b_treetop_detection_lidar.py --radius 1.25 --minheight 2.0
    python scripts/02b_treetop_detection_lidar.py --radius 2.5 --minheight 3.0

Arguments:
    --radius        float, search radius in metres for local maximum
                    detection (default: 1.25, equivalent to window=5
                    at 50cm resolution)
...
LAYER_NAME_TPL = "treetops_lidar_r{radius}_mh{minheight}"
# ============================================================
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.25,
        help="Search radius in metres for local maximum detection "
             "(default: 1.25 = equivalent to window=5 at 50cm res)"
    )
    parser.add_argument(
        "--minheight",
        type=float,
...
if __name__ == "__main__":
    ...
    try:
        logger.info("=" * 60)
        logger.info("STEP 02b: Tree top detection from point cloud")
        logger.info("=" * 60)
        ...
        is_local_max = detect_local_maxima_3d(xs, ys, zs, args.radius, logger)
        ...
        logger.info("=" * 60)
        logger.info("Next step: run 03_crown_segmentation.py with")
        logger.info(f"  --treetops-layer {layer_name}")
        logger.info(f"  --treetops-gpkg  {OUTPUT_GPKG}")
